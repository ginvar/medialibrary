package com.ginvar.library.datamanager;
...
import com.ginvar.library.mediarecord.audio.AudioRecordWrapper;
import com.ginvar.library.mediarecord.utils.MediaUtils;
...
public class AudioDataManager {
    ...
    private int mWriteIndex = 0;
            // global write index in each Audio segment, start from 0
    private int mReadIndex = 0;
            // global read index in each Audio segment,start from 0
    ...
    public void startRecord() {
        ...
        mWriteIndex = 0;
        mInited.set(true);
        Log.i(TAG, "Audio segment [" + mCurWriteSegIndex + "] record start.");
    }
    ...
    public void stopRecord() {
        ...
        if (mWriteIndex == 0) {  // started, but no frame write
            ...
            Log.i(TAG, "Audio segment [" + mCurWriteSegIndex + "] record stop with no frame write, drop it.");
            return;
        }
        ...
        if (packet != null) {
            ...
            Log.i(TAG, "mCurWriteSegment [" + mCurWriteSegIndex + "] end  pts " + packet.pts);
        }
        ...
        if (mAudioMediaFormat != null) {
            ...
            Log.i(TAG, "Audio segment [" + mCurWriteSegIndex + "] end index " + mCurWriteSegment.mEndIndex +
                    " duration " + (mCurWriteSegment.mEndPts - mCurWriteSegment.mStartPts) + " Total duration " +
                    mDuration + " frame count " + mTotalFrameCnt);
        }
    ...
    public void writeMediaFormat(MediaFormat format) {
        if (format == null) {
            Log.e(TAG, "writeMediaFormat error ! format == null");
            return;
        }
        ...
        if (mAudioMediaFormat == null) {
            ...
            mAudioMediaFormat = MediaUtils.createAacFormat(sampleRate, channels);
        }
    }
    ...
    public int write(YYAudioPacket packet) {
        ...
        if (packet.mBufferSize > 0) {
            ...
            if (mCurWriteSegment.mStartPts == -1) {
                mCurWriteSegment.mStartPts = packet.pts;
                Log.i(TAG, "mCurWriteSegment.mStartPts " + packet.pts);
            }
            mCurWriteSegment.mEndPts = packet.pts;
            mCurWriteSegment.mEndIndex = mWriteIndex;
//            Log.i(TAG, "Write Seg " + mCurWriteSegIndex +" index " + mWriteIndex + " len " +
//                    packet.mBufferSize + " pts " + packet.pts + " ori pts " + origPts);
    ...
    private int findAudioPacketByPts(AudioSegment segment, long pts) {
        ...
        //Log.i(TAG, "jtzhu findAudioPacketByPts l " + l + " pts " + mAudioDataMap.get(l).pts +
        // " r " + r + " pts " + mAudioDataMap.get(r).pts + " target " + pts + " result " + result);
        return labs > rabs ? r : l;
    }

//    private YYAudioPacket readByPts(long pts) {
//        if (pts > mEndPTS || pts < mStartPTS) {
//            Log.e(TAG, " read error ! pts " + pts + " start pts " + mStartPTS + " end pts " + mEndPTS);
//            return null;
//        }
...
//    }
    ...
    private YYAudioPacket readByIndex(int index) {
        ...
        if (index < 0 || index > mCurReadSegment.mEndIndex || mCurReadSegment.mAudioDataMap == null) {
            Log.w(TAG, " end of Audio segment [" + mCurReadSegIndex + "] index " + index +
        ...
        if (mAudioSegmentIndexToDelete != -1 && mAudioIndexToDelete != -1) {
            if (mCurReadSegIndex == mAudioSegmentIndexToDelete && index >= mAudioIndexToDelete) {
                Log.w(TAG, " end of Audio segment [" + mCurReadSegIndex + "] index " + index +
                        " mAudioSegmentIndexToDelete " + mAudioSegmentIndexToDelete + " mAudioIndexToDelete " +
                        mAudioIndexToDelete);
                return null;
            }
        }
    ...
    public void seekTo(long timeUs, int mode) {
        ...
        if (segment != null) {
            ...
            Log.i(TAG, " seekto " + original + "segment index " + mCurReadSegIndex + " mReadIndex " + mReadIndex +
                    " mStartPTS " + mStartPTS + " mEndPTS " + mEndPTS + " mode " + mode);
        } else {
            Log.e(TAG, "seekTo " + timeUs + " error! ");
        }
    }
    ...
    public YYAudioPacket readSampleData() {
        ...
        //Log.i(TAG, "jtzhu read sample index : " + mReadIndex );
    ...
    public boolean advance() {
        ...
        if (mReadIndex > mCurReadSegment.mEndIndex) {
            ...
            if (newSeg == null) {
                Log.i(TAG, "end of Audio,mCurReadSegIndex " + mCurReadSegIndex + " mReadIndex " + mReadIndex);
                return false;
            }

            mCurReadSegment = newSeg;
            mReadIndex = 0;
            Log.i(TAG, "new Read segment index " + mCurReadSegIndex + " mReadIndex " + mReadIndex);
        }
        //long pts = getSampleTime();
        //Log.i(TAG, "advanced mCurReadSegIndex " + mCurReadSegIndex +
        // " mReadIndex " + mReadIndex + " pts " + pts);
        return true;
    }
    ...
    public void removeSegmentByIndex(int segIndex) {
        Log.i(TAG, "removeSegmentByIndex segIndex " + segIndex);
        if (segIndex < 0) {
            Log.e(TAG, "removeSegmentByIndex segIndex " + segIndex + " error !");
            return;
        }
        AudioSegment segment = null;
        ...
        if (segment != null) {
            ...
            adjustPts(segment);
            Log.i(TAG, " removeSegmentByIndex " + segIndex + " OK.");
            if (segment == mLastWriteSegment) {                    // 删除一段后，更新最后一段的位置
                ...
                if (mLastWriteSegment != null) {
                    ...
                    Log.i(TAG,
                            " new mLastWriteSegment index " + mLastWriteSegment.mSegmentIndex + " new writeSegIndex " +
                                    mCurWriteSegIndex);
                } else {
                    Log.i(TAG, " new mLastWriteSegment index  -1. ");
                }
            }
            mAudioSegmentList.remove(segment);
        } else {
            Log.i(TAG, "removeSegmentByIndex not found segment for segIndex " + segIndex);
        }
    }
    ...
    public boolean advanceForExport() {
        ...
        if (mReadindexForExport > mCurReadSegmentForExport.mEndIndex) {
            AudioSegment newSeg = null;
            while (mCurReadSegIndexForExport <
                    mTotalSegmentCnt) { // move to the next available Audio segment, may be deleted
            ...
            if (newSeg == null) {
                Log.i(TAG, "end of Audio,mCurReadSegIndexForExport " + mCurReadSegIndexForExport +
                        " mReadIndexForExport " + mReadindexForExport);
                return false;
            }

            mCurReadSegmentForExport = newSeg;
            mReadindexForExport = 0;
            Log.i(TAG, "new Read segment index " + mCurReadSegIndexForExport +
                    " mReadIndexForExport " + mReadindexForExport);
        }
        //long pts = getSampleTime();
        //Log.i(TAG, "advanced mCurReadSegIndex " + mCurReadSegIndex +
    ...
    public void seekToForExport(long timeUs, int mode) {
        ...
        if (segment != null) {
            ...
            Log.i(TAG, " seekToForExport " + original + "segment index for Export " +
                    mCurReadSegIndexForExport + " mReadIndexForExport " + mReadindexForExport +
                    " mStartPTS " + mStartPTS + " mEndPTS " + mEndPTS + " mode " + mode);
        } else {
            Log.e(TAG, "seekTo " + timeUs + " error! ");
        }
    ...
    public YYAudioPacket readSampleDataForExport() {
        ...
        //Log.i(TAG, "jtzhu export segment:" + mCurReadSegIndexForExport + " index : " + mReadindexForExport );
    ...
    private YYAudioPacket readByIndexForExport(int index) {
        ...
        if (index < 0 || index > mCurReadSegmentForExport.mEndIndex || mCurReadSegmentForExport.mAudioDataMap == null) {
            Log.w(TAG, " end of Audio segment [" + mCurReadSegIndexForExport + "] index " + index +
    ...
    public int exportAudioToMp4(String path) {
        if (mAudioSegmentList.size() <= 0 || mDuration <= 0) {
            Log.e(TAG, "No Audio Data in Memory !");
            return -1;
        }
        if (path == null) {
            Log.e(TAG, "path == null !");
        ...
        try {
            ...
            try {
                ...
                Log.e(TAG, "muxer.start exception : " + e.getMessage());
            }
            seekToForExport(0, 0);
            while (true) {
                ...
                if (null != packet) {
                    ...
                    //Log.e(TAG, "write to Mp4 size : " + packet.mBufferSize + " offset " +
                    // packet.mBufferOffset + " pts " + packet.pts );
                    ...
                    try {
                        ...
                        Log.e(TAG, "IllegalStateException " + e.toString() + " " + e.getMessage());
                    } catch (IllegalArgumentException e) {
                        Log.e(TAG, "IllegalArgumentException " + e.toString() + " " + e.getMessage());
                    }
                    advanceForExport();
                    audioFrameCount++;
                } else {
            ...
            try {
                ...
                Log.e(TAG, "muxer.stop exception :" + e.getMessage());
            }
            muxer.release();
            Log.i(TAG, "exportAudioToMp4 cost " + (System.currentTimeMillis() - time));
        } catch (IOException e) {
    ...
    public void markTimePointToDelete(int segIndex, long timeMs) {
        ...
        Log.i(TAG, "markTimePointToDelete segIndex " + segIndex + " timeMs " + timeMs);
        for (AudioSegment seg : mAudioSegmentList) {
            if (seg.mSegmentIndex == segIndex) {
                ...
                if (timeMs <= seg.mEndPts) {
                    ...
                    Log.i(TAG, "markTimePointToDelete " + mTimePointToDelete + " mAudioIndexToDelete " +
                            mAudioIndexToDelete);
                } else {
                    Log.i(TAG, "markTimePointToDelete error " + timeMs +
    ...
    private void deleteToEndInSeg(AudioSegment seg, int startIndex) {
        ...
        mDuration = calculateTotalDuration();
        mAudioMediaFormat.setLong(MediaFormat.KEY_DURATION, mDuration);
        Log.i(TAG, "doDeleteInSegment segIndex " + segIndex + " new seg.mEndIndex " + seg.mEndIndex + " endPts " +
                seg.mEndPts);
    }
    ...
    public void doDeleteInLastSegment() {
        ...
        Log.i(TAG,
                "doDeleteInSegment segIndex " + mAudioSegmentIndexToDelete + " videoIndex " + mAudioIndexToDelete);
